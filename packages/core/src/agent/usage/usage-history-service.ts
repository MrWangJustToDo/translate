 ...
 *
 * A thin composed wrapper on the {@link UsageStore} pure-IO layer. All agents
 ...
 *
 * Lives next to its IO layer (`usage-store.ts`) — not under managers/ — so
 * domain modules (models/, agent/) can import the shared instance without
 * violating the `models→managers` / `agent→managers` boundary gates.
 *
...
import type { UsageHistoryResult, UsageRecordInput } from "./usage-store.js";

export type { UsageHistoryResult, UsageRecord, UsageRecordInput } from "./usage-store.js";
