/**
 * usage-history.ts - Global usage history store (contribution-graph source).
 *
 ...
 *
 * Write path: append-only, serialized through an in-module promise chain
 * (mirrors session-journal.ts). Query path: scan the year files that overlap
 * the requested range and aggregate into per-day buckets plus per-model totals.
 */
...
/**
 * Append one usage record. Fire-and-forget safe: failures are swallowed (a
 * lost telemetry line must never break an agent run), appends are serialized
 * so concurrent calls cannot interleave partial lines.
 */
export function recordUsage(input: UsageRecordInput): void {
  const record = buildRecord(input);
  if (!record) return;
  writeChain = writeChain.then(() => appendRecord(record)).catch(() => {});
}
...
async function appendRecord(record: Omit<UsageRecord, "v">): Promise<void> {
  const fs = getEnv().fs;
  if (!fs.appendFile) return;
...
async function listUsageLogFiles(fs: ReturnType<typeof getEnv>["fs"]): Promise<string[]> {
...
/**
 * Read usage records for the last `weeks` weeks (ending today, local timezone)
 * and aggregate into per-day buckets + per-model totals.
 */
export async function getUsageHistory(weeks: number): Promise<UsageHistoryResult> {
  const fs = getEnv().fs;
  const result: UsageHistoryResult = { daily: [], models: [], skipped: 0 };
  const files = await listUsageLogFiles(fs);
